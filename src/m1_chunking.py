...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    # Try semantic embedding, fallback to paragraph-based if unavailable
    try:
        ...
        # Fallback: paragraph-based chunking when sentence-transformers unavailable
        paragraphs = [p.strip() for p in text.split("\n\n") if p.strip()]
        return [Chunk(text=p, metadata={**metadata, "strategy": "semantic-fallback"}) for p in paragraphs]
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current_parent) + len(para) > parent_size and current_parent:
            ...
            sub_paras = [p.strip() for p in parent_text.split("\n\n") if p.strip()]
            current_child = ""

            for sub_p in sub_paras:
                if len(current_child) + len(sub_p) > child_size and current_child:
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    if current_content:
        ...
        chunks.append(Chunk(
            text=chunk_text,
            metadata={**metadata, "section": current_header.strip() if current_header else "", "strategy": "structure"}
        ))

    # Handle case with no headers at all
